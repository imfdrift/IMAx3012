...
Skisser gradientvektorfeltet og nivåkurvene til $f(x, y) = x^2 + y^2$ (se \Cref{sec:nivakurver-rep} for en repetisjon av nivåkurver). Nivåkurver er gitt av:
\[
...
For en funksjon $f$ av reelle verdier gjelder for vektoren $\nabla f$ at den
\begin{itemize}
\item i hvert punkt peker i retning av størst endring av $f$.
...
\vect{F} = \nabla f
\]
Da er $f$ en \emph{potensialfunksjon} for $\vect{F}$. På kortform sier man at $f$ er et potensiale for vektorfeltet $\vect{F}$.
\end{defn}
...
$ (Se JuPyter-notat for visualisering).
...
Dersom $\text{div } \vect{F} < 0$, betyr det at gassen (eller væsken) komprimeres (eller flyter inn i et sluk).
...
\begin{eks}{Sentralsymmetriske vektorfelt i $d$ dimensjoner}
Betrakt et \emph{sentralsymmetrisk} (radielt) vektorfelt på formen
\[
\vect{F}(\vect{r}) = \frac{C}{r^n}\,\hat{\vect{r}}, \qquad \vect{r}=(x_1,\ldots,x_d)\in\R^d,\quad r=\lVert\vect{r}\rVert,\quad \hat{\vect{r}}=\vect{r}/r,
\]
hvor $C$ er en konstant, $d$ er antall dimensjoner og $n$ er en eksponent. Man kan vise at
\[
\operatorname{div}\vect{F} = \nabla\cdot\vect{F} = (d-n-1)\,\frac{C}{r^{n+1}}.
\]
Dette betyr at $\operatorname{div}\vect{F}=0$ overalt på definisjonsmengden $\R^d\setminus\{\vect{0}\}$ nøyaktig når $n=d-1$:
\begin{itemize}
\item[$\bullet$] for $d=2$: da er $\vect{F} = \dfrac{C}{r}\,\hat{\vect{r}}$ divergensfritt,
\item[$\bullet$] for $d=3$: da er $\vect{F} = \dfrac{C}{r^2}\,\hat{\vect{r}}$ divergensfritt -- dette er nøyaktig formen til blant annet Coulombs lov og Newtons gravitasjonslov!
\end{itemize}

\paragraph{Kommentar:} Et vektorfelt på formen $\vect{F}=C\vect{r}/r^{n+1}$ er altså ikke generelt sett divergensfritt. At divergensen forsvinner er en direkte konsekvens av $n= d-1$ . Avtagningsraten balanserer nøyaktig mot hvordan "flaten" til en kule av radius $r$ i $\R^d$ vokser med $r^{d-1}$.
\end{eks}
...
Ettersom curl er definert som et kryssprodukt, kan vi se at curlen til et vektorfelt i $\R^3$ alltid er ortogonal på vektorfeltet. Dette betyr at curlen til et vektorfelt i $\R^3$ alltid vil være et vektorfelt som står vinkelrett på det opprinnelige feltet. I tillegg kan vi legge merke til at curl kun er definert i tre dimensjoner, i motsetning til gradient og divergens som er definert i alle dimensjoner. 
...
Eksperimenter viser at væske som renner ut av et badekar vanligvis er irrotasjonell — bortsett fra helt i sentrum — selv om væsken “roterer” rundt sluket. 
...
\[\nabla\times\vect{F}=0.\]
Samtidig ser man tydelig fra figuren at dette vektorfeltet ``roterer'' rundt origo. Akkurat i origo er ikke $\nabla\times\vect{F}=0$, men origo er ikke med i definisjonsmengden til vektorfeltet. Rotasjonen er derfor rundt et punkt som ikke er med i definisjonsmengden. Ordet \emph{irrotasjonell} kan derfor skape forvirring: En må se nøye på definisjonsmengden for å se om et vektorfelt er rotasjonsfritt eller ei.
\end{eks}
\begin{figure}[htpb]
    \centering
...
\begin{oppsum}[Noen identiteter]\,
Vi har nå sett to identiteter som knytter sammen de klassiske differensialoperatorene. For det første er curl av et gradientfelt alltid null:
\[\nabla\times(\nabla f)=\vect{0},\]
forutsatt at $f(x,y,z)$ har kontinuerlige deriverte av andre orden. På samme måte kan en enkelt vise at divergensen av curl til et vektorfelt $\vect{F}(x,y,z)$ alltid er null:
\[\nabla\cdot\left(\nabla\times\vect{F}\right)=0.\]
Begge identitetene følger av matematiske egenskaper ved de partiellderiverte, og gir god fysisk mening: Det er ingen spredning i rotasjon, og ingen rotasjon i spredning.
\end{oppsum}
...
\end{defn}
\begin{rem}{}
Alle differensialoperatorer beskrives er her beskrevet i kartesiske koordinater. I andre koordinatsystemer ser de annerledes ut. Laplace i polarkoordinater er for eksempel gitt som 
...
\paragraph{Identiteter: }Det finnes massevis identiteter som knytter de klassiske differensialoperatorene sammen. I \Cref{app:diffop_ident} finnes det en overblikkstabell med de vanligste identitetene.
...
\int_C \vect{F} \cdot d\vect{r} = \int_a^b \vect{F}(\vect{r}(t)) \cdot \vect{v}(t)\, dt \quad \text{ for } \vect{v}(t)=\vect{r}\,'=(r_1'(t),r_2'(t),\ldots, r_n'(t))^\top
\end{align}
...
\end{defn}
Mer konkret: Gitt vektorfeltet 
\[
\vect{F}(x, y, z) = P(x, y, z)\, \vect{i} + Q(x, y, z)\, \vect{j} + R(x, y, z)\, \vect{k}
\]
i $\R^3$, samt en glatt kurve \( C \) med parametrisering
\[
...
For linjeintegralet av en funksjon over en parameter-kurve fant vi at integrasjonsretningen ikke har noe å si. Men merk at dette ikke stemmer for linjeintegraler av vektorfelt. Grunnen til dette er at vektorfeltet i seg selv har retning. Dermed vil integrasjonsretningen ha betydning for fortegnet til integralet:
\begin{empheq}[box=\colorbox{gray!15}]{equation*}
...
\[
dW = \vect{F} \cdot \vect{r} = \vect{F} \cdot \vect{v}(t) \, dt.
\]
Hvor $\vect{v}(t) = \vect{r}'(t)$ er hastighetsvektoren. Som vanlig summerer vi alle de små bidragene og tar grensen når lengden av bitene går mot null, noe som gir et integral.
...
\[
\text{Arbeid} = \int_C \vect{F} \cdot d\vect{r} = \int_a^b \vect{F}(\vect{r}(t)) \cdot \vect{v}(t) \, dt.\] 
hvor $\vect{v}(t) = \vect{r}'(t)$ er hastighetsvektoren.
\end{defn}
...
\end{align*}
\textbf{Alternativt:} Bevegelsen skjer langs en sirkel med posisjonvektor $\vect{r}(t)$, og hastighetsvektoren (tangenten) er alltid vinkelrett på $\vect{r}$. Dermed er skalarproduktet $\vect{F} \cdot d \vect{r} = 0$, siden kraften peker radielt og bevegelsen er tangentiell. Derfor er arbeidet null.
\end{eks}
...
Analysens fundamentalteorem for vanlige funksjoner sier at integralet av en deriverte over et intervall er lik differansen mellom funksjonsverdiene i intervallets endepunkter:
...
\newpage
\begin{defn}{}
La \( \vect{F} \) være et kontinuerlig vektorfelt i en mengde \( D \). Anta at $C$ er en kurve i $D$ parametrisert med $\vect{r}$. Vi sier  at \( \int_C \vect{F} \cdot d\vect{r} \) er \emph{uavhengig av sti} hvis den samme verdien oppnås for enhver kurve $\tilde{C} \subseteq D$ med samme start- og sluttpunkt som $C$. 
\end{defn}

Spørsmålet er nå om vi kan karakterisere integraler som er uavhengig av sti. Det er hensiktsmessig å introdusere noen navn for spesielle mengder og kurver når vi er interessert i integraler uavhengig av sti.
...
Vi skal møte lukkete og enkle kurver og linjeintegraler langs dem ofte siden de utgjør randen til enkle områder i planet.
 ...
 \item \textbf{stjerneformet} hvis det finnes et punkt $\star \in D$ slik at for alle andre punkter $P \in D$ er linjestykket mellom $\star$ of $P$ fullstendig i $D$. Punktet $\star$ kaller man da\emph{stjernepunkt} for $D$.
\end{itemize}
\end{defn}
...
\begin{enumerate}
    \item Hvis \( \vect{F} = \nabla f \) (dvs. vektorfeltet er konservativt), så er \( \int_C \vect{F} \cdot d\vect{r} \) sti-uavhengig. 
...
\end{enumerate}
\textbf{OBS:} På åpne og stjerneformede mengder er linjeintegraler av vektorfelt uavhengig av sti hvis og bare hvis vektorfeltet er konservativt.
\end{oppsum}
...
= 2\pi \ne 0.
\end{align*}
Siden vi har funnet en lukket kurve der integralet av $\vect{F}$ ikke er null, kan ikke $\vect{F}$ være konservativt. Det som skjer her er at vektorfeltet er definert på et område som ikke er stjerneformet, og dermed kan vi ikke bruke \Cref{thm:kons_stjern_2D} til å konkludere at $\vect{F}$ er konservativt.
\end{eks}
...
Bestem om vektorfeltet $\vect{F}(x,y) = \frac{-y\vect{i}+x\vect{j}}{x^2+y^2}$ på det \textit{høyre halvplanet},
\[
D = \{(x, y) \in \mathbb{R}^2 \mid x > 0\}.
\] er konservativt, og finn i så fall en potensialfunksjon.
