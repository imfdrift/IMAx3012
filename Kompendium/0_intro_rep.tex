...
\section{$\R^n$ og notasjon for vektorer}
...
\section{Kontinuerlige funksjoner}
...
\section{Parametrisering av linjestykker}
...
\]
Merk da at $\vect{d}$ er den deriverte: 
\[\vect{r}'(t)=\frac{\text{d}}{\text{dt}}\vect{r}(t)=\vect{d}\]
...
\]
Vektoren $(x'(t),y'(t))$ peker langs kurven, akkurat som $\vect{r}'(t)$ gjorde for det parametriserte linjestykket tidligere -- den er altså en retningsvektor for tangenten til nivåkurven. Skalarproduktet over er null, så akkurat som før vet vi da at vektoren
\[
(f_x,\,f_y) \ \text{står vinkelrett på denne tangenten.}
\]
Med andre ord: \emph{vektoren av partiellderiverte $(f_x,f_y)$ står alltid normalt på nivåkurvene til $f$.} Siden skal vi lære at denne vektoren kalles for \textit{gradienten} til funksjonen $f$. 
...
\section{Likningen for et plan}
\label{sec:plan-likning}

Et plan i rommet er entydig bestemt av ett punkt $P_0=(x_0,y_0,z_0)$ på planet, og én vektor $\vect{n}=(a,b,c)^\top$ som står \emph{normalt} på planet. Ideen er akkurat den samme som for normallinjen til en kurve i forrige avsnitt: dersom $P=(x,y,z)$ er et \emph{vilkårlig} annet punkt på planet, så ligger vektoren $\vect{P_0P}=P-P_0=(x-x_0,\,y-y_0,\,z-z_0)^\top$ \emph{i} planet, og må derfor stå vinkelrett på normalvektoren $\vect{n}$. Med skalarproduktet betyr dette at
\[
\vect{P_0P}\cdot\vect{n} = 0.
\]

\begin{center}
\begin{tikzpicture}[scale=1.0]
  % Parallellogram som representerer en bit av planet
  \coordinate (A) at (0,0);
  \coordinate (B) at (4,0.3);
  \coordinate (D) at (1,2.0);
  \coordinate (C) at (5,2.3);

  \filldraw[fill=blue!8, draw=blue!55!black, thick] (A) -- (B) -- (C) -- (D) -- cycle;

  % Punktene P0 og P i planet
  \coordinate (P0) at (2.2,0.935);
  \coordinate (P) at (4.15,1.755);

  \draw[thick, -latex, black] (P0) -- (P) node[midway, below right, font=\footnotesize] {$\vect{P_0P}$};

  \filldraw (P0) circle (1.3pt);
  \node[below, font=\footnotesize] at (P0) {$P_0$};
  \filldraw (P) circle (1.3pt);
  \node[right, font=\footnotesize] at (P) {$P$};

  % Normalvektoren n
  \coordinate (N) at (1.9,2.635);
  \draw[thick, -latex, red] (P0) -- (N);
  \node[above, font=\footnotesize, text=red] at (N) {$\vect{n}$};
\end{tikzpicture}
\captionof{figure}{En bit av planet (som i virkeligheten strekker seg uendelig langt i alle retninger), bestemt av punktet $P_0$ og normalvektoren $\vect{n}$ (rød), som står vinkelrett på planet. Et vilkårlig annet punkt $P$ på planet gir en vektor $\vect{P_0P}$ (svart) som ligger i planet, og derfor er vinkelrett på $\vect{n}$.}
\label{fig:plan-normal}
\end{center}

\begin{defn}{Likningen for et plan}
Planet gjennom punktet $P_0=(x_0,y_0,z_0)$ med normalvektor $\vect{n}=(a,b,c)^\top$ består av alle punkter $P=(x,y,z)$ som oppfyller
\[
\vect{n}\cdot \vect{P_0P} = a(x-x_0)+b(y-y_0)+c(z-z_0) = 0.
\]
Dette kan også skrives på formen
\[
ax+by+cz=d, \qquad \text{der } d = ax_0+by_0+cz_0.
\]
\label{defn:plan-likning}
\end{defn}

\begin{rem}{}
Ofte er ikke normalvektoren $\vect{n}$ gitt direkte, men vi kjenner i stedet to vektorer $\vect{u}$ og $\vect{v}$ som begge ligger \emph{i} planet (for eksempel utspent av tre kjente punkter i planet). Da kan vi finne $\vect{n}$ ved hjelp av kryssproduktet (\Cref{defn:vektorprodukt}):
\[
...
\begin{eks}{Likningen for planet gjennom tre punkter}
La oss finne likningen for planet gjennom punktene
\[
A=(1,0,0), \qquad B=(0,1,0), \qquad C=(0,0,1).
\]

\textbf{Steg 1: Finn to vektorer i planet.} Vi bruker $A$ som utgangspunkt, og danner
\[
\vect{u} = \vect{AB} = B-A = (-1,1,0)^\top, \qquad \vect{v} = \vect{AC} = C-A = (-1,0,1)^\top.
\]

\textbf{Steg 2: Finn normalvektoren ved kryssprodukt.}
\[
\vect{n} = \vect{u}\times\vect{v} = \begin{vmatrix}\vect{i} & \vect{j} & \vect{k} \\ -1 & 1 & 0 \\ -1 & 0 & 1\end{vmatrix} = (1\cdot 1 - 0\cdot 0)\,\vect{i} - \bigl((-1)\cdot 1 - 0\cdot(-1)\bigr)\,\vect{j} + \bigl((-1)\cdot 0 - 1\cdot(-1)\bigr)\,\vect{k} = (1,1,1)^\top.
\]

\textbf{Steg 3: Sett opp likningen.} Med $P_0=A=(1,0,0)$ og $\vect{n}=(1,1,1)^\top$ gir \Cref{defn:plan-likning}
\[
1\cdot(x-1) + 1\cdot(y-0) + 1\cdot(z-0) = 0 \quad\Longleftrightarrow\quad x+y+z=1.
\]
Vi kan lett sjekke at alle de tre punktene $A,B,C$ oppfyller denne likningen. \checkmark
\end{eks}

\section{Integrasjon av énvariabelfunksjoner}
...
\label{sec:polarkoord-rep}
Foruten kartesiske koordinater $(x,y)$ kommer vi til å bruke polarkoordinater flere steder i kurset (se f.eks.\ \Cref{sect:polarkoord}), så vi repeterer helt kort det mest grunnleggende her.

\begin{center}
\begin{tikzpicture}[scale=0.85]
  % --- polarkoordinatnett (tegnes først, under kartesisk system) ---
  \foreach \rad in {1,2,3,4}{
    \draw[gray!35, thin] (0,0) circle (\rad);
  }
  \foreach \ang in {0,30,...,330}{
    \draw[gray!25, thin] (0,0) -- (\ang:4.3);
  }

  % --- kartesiske akser oppå polarnettet ---
  \draw[-latex, thick] (-4.6,0) -- (4.6,0) node[right] {$x$};
  \draw[-latex, thick] (0,-4.6) -- (0,4.6) node[above] {$y$};

  % --- punktet P ---
  \def\rP{3.3}
  \def\angP{50}
  \pgfmathsetmacro{\xP}{\rP*cos(\angP)}
  \pgfmathsetmacro{\yP}{\rP*sin(\angP)}
  \coordinate (O) at (0,0);
  \coordinate (P) at (\angP:\rP);

  % projeksjonslinjer (kartesiske koordinater til P)
  \draw[dashed] (P) -- (\xP,0) node[below, font=\footnotesize] {$x_1$};
  \draw[dashed] (P) -- (0,\yP) node[left, font=\footnotesize] {$y_1$};

  % vinkelbue
  \draw[thick, blue] (0.9,0) arc (0:\angP:0.9);
  \node[blue, font=\footnotesize] at ({\angP/2}:1.3) {$\theta$};

  % radiuspil
  \draw[-latex, thick, red] (O) -- (P);
  \node[red, font=\footnotesize] at ({\angP/2+8}:2.0) {$r$};

  % punktet og etiketten
  \filldraw[green!45!black] (P) circle (1.6pt);
  \node[green!45!black, above right, font=\footnotesize] at (P) {$P[r,\theta]=P(x_1,y_1)$};

  % polaraksen
  \draw[-latex, orange!70!black, thin] (3.9,-0.55) -- (3.75,-0.05);
  \node[below, font=\footnotesize, text=orange!70!black] at (3.9,-0.6) {polaraksen};

  % origo
  \node[below left, font=\footnotesize] at (O) {$O$};
\end{tikzpicture}
\captionof{figure}{Et punkt $P$ angitt både med kartesiske koordinater $(x_1,y_1)$ og polarkoordinater $P[r,\theta]$. Polarkoordinatnettet (grå sirkler og stråler) er her lagt oppå det kartesiske koordinatsystemet, med polaraksen langs den positive $x$-aksen.}
\label{fig:polarkoord-rep}
\end{center}

\begin{defn}{Polarkoordinater}
Et punkt $P$ i planet kan angis ved polarkoordinatene $P[r,\theta]$, der
\begin{itemize}
\item $r$ er avstanden fra origo $O$ til $P$,
\item $\theta$ er vinkelen mellom polaraksen (den positive $x$-aksen) og linjestykket $OP$, målt mot klokka.
\end{itemize}
\end{defn}
...
\begin{defn}{Sammenheng mellom kartesiske og polare koordinater}
\[
